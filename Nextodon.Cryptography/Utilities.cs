...
internal static class Utilities
{
    ...
    public static byte[] GetRandomBytes(int size, int seedStretchingIterations = 5000)
    {
        ...
        byte[] threadedSeedBytes = new Portable::Org.BouncyCastle.Crypto.Prng.ThreadedSeedGenerator().GenerateSeed(24, true);
    ...
    {
        // Normalize.
        var buffer = toNormalise.Normalize(NormalizationForm.FormKD);

        char[] trim = { '\0' };

        return buffer.TrimEnd(trim);
    }
    ...
    private static string NormalizeString(NormalizationForm NormForm, string lpSrcString)
    {
        return lpSrcString.Normalize(NormForm);
    }
}
